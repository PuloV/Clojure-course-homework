...
(deftest task-02-sample-test
  ...
  (is (= (search collection :artist {:genre "Jazz"})#{"John Coltrane" "Miles Davis" "Bill Evans"}))
  (is (= (search collection :subgenre {:artist "John Coltrane"})#{"Bebop" "Avantgarde"}))
  (is (= (search collection :tags {:artist "John Coltrane"}) #{:jazz :bebop :popular :cover :avantgarde}))
  (is (= (search collection :title [:and {:genre "Classical"} {:tag :violin}])#{"Eine Kleine Nachtmusik"}))
